$(function(){
	...
	$('.form_group').on('click focusin',function(){
	    ...
	    // $(this).siblings('input').focus().parent().addClass('hotline');
	})
    ...
    $(".login_form_con").submit(function (e) {
        ...
        $.ajax({
            ...
            // 在header 中添加csrf_token的随机值
            headers: {
                "X-CSRFToken": getCookie("csrf_token")
            },
            //　在ｈｅａｄｅｒ中添加csrf_token的随机值
...
function sendSMSCode() {
    ...
    $.ajax({
        ...
        success:function (resp) {
            if (resp.errno == "0"){
                //倒计时60秒, 60秒之后允许用户再次点击发送短信验证码的按钮
                var num = 60;
                //设置一个计时器
                ...
                //表示后端出现了错误，　可以将错误信息展示到前端页面
                $("#register-sms-code-err").html(resp.errmsg);
                $("#register-sms-code-err").show();
                //将点击按钮的onclick事件函数恢复回去
                $(".get_code").attr("onclick", "sendSMSCode();");
                //如果错误代码是4004, 代表验证码错误，重新生成验证码
                if (resp.errno == "4004"){
                    generateImageCode()
                }
            }
